...
import { Field, FieldError, FieldGroup, FieldLabel } from "@kumix/ui/ui/field";
import { InputOTP, InputOTPGroup, InputOTPSeparator, InputOTPSlot } from "@kumix/ui/ui/input-otp";
import { Separator } from "@kumix/ui/ui/separator";
import { Spinner } from "@kumix/ui/ui/spinner";
import { cn } from "@kumix/utils";
import { useAuthForm } from "../auth-form";
import { deviceAuthorizationPlugin } from "../lib/device-authorization-plugin";
      ...
      <CardContent>
        <div className="flex flex-col gap-3 rounded-lg border bg-muted/50 p-3">
          <div className="flex flex-col gap-1">
            <p className="text-muted-foreground text-xs">{localization.deviceCode}</p>
            <p className="font-medium font-mono text-sm tracking-wider">{userCode}</p>
          </div>

          <Separator />

          <div className="flex flex-col gap-1">
            <p className="text-muted-foreground text-xs">{localization.signedInAs}</p>
            <p className="font-medium text-sm">{user.name || user.email}</p>
            {user.name ? <p className="text-muted-foreground text-xs">{user.email}</p> : null}
          </div>
        </div>
      </CardContent>
