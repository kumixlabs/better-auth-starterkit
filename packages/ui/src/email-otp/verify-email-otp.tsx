...
import { Field, FieldDescription, FieldGroup, FieldLabel } from "@kumix/ui/ui/field";
...
import { RESEND_COOLDOWN_SECONDS, useResendCooldown } from "../lib/use-resend-cooldown";
...
export function VerifyEmailOtp({ className }: VerifyEmailOtpProps) {
  ...
  const { mutateAsync: sendVerificationOtp, isPending: isSending } = useSendVerificationOtp(
    otpClient,
    {
      onSuccess: (_data, { email: sentTo }) => {
        ...
        setEmail(sentTo);
        startCooldown();
        toastSuccess({ message: emailOtpLocalization.codeSent });
      },
    },
  );
  ...
  const { mutateAsync: verifyEmailOtp, isPending: isVerifying } = useVerifyEmailOtp(otpClient, {
    ...
    onSuccess: () => {
      ...
      toastSuccess({ message: emailOtpLocalization.emailVerified });
