...
import { InputGroup, InputGroupAddon, InputGroupInput } from "@kumix/ui/ui/input-group";
import { Spinner } from "@kumix/ui/ui/spinner";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@kumix/ui/ui/table";
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{localization.rolesDescription}</p>
        </div>
      ...
      {canRead.isPending || roles.isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col gap-1">
            ...
            <p className="text-muted-foreground text-sm">{localization.noRolesDescription}</p>
          </CardContent>
        </Card>
      )}
...
function RoleDialog({
  ...
  const createRole = useCreateRole(authClient, organizationId, {
    onSuccess: () => {
      toastSuccess({ message: localization.roleCreated });
      onOpenChange(false);
    },
  });
  const updateRole = useUpdateRole(authClient, organizationId, {
    onSuccess: () => {
      toastSuccess({ message: localization.roleUpdated });
      onOpenChange(false);
    },
  });
      ...
      try {
        if (Object.values(value.permission).some((actions) => actions.length > 0)) {
          ...
          if (access.error || !access.data?.success) {
            toastError({ message: localization.permissionsLimitedDescription });
            return;
          }
        }
            ...
            <form.AppField name="permission">
              {(field) => (
                <fieldset className="flex flex-col gap-4">
                  <legend className="font-medium text-sm">{localization.permissions}</legend>
                  {Object.entries(registry).map(([resource, definition]) => (
                    <div className="flex flex-col gap-2" key={resource}>
                      <p className="font-medium text-sm">{definition.label ?? resource}</p>
