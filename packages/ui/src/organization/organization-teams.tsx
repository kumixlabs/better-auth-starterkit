    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="text-muted-foreground text-sm">{localization.teamsDescription}</p>
        </div>
      ...
      {teams.isPending ? (
            ...
            {teams.data.map((team) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-muted-foreground text-sm">{localization.team}</p>
                </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-1">
            ...
            <p className="text-muted-foreground text-sm">{localization.noTeamsDescription}</p>
          </CardContent>
        </Card>
      )}
...
function TeamDialog({
  ...
  const createTeam = useCreateTeam(authClient, {
    onSuccess: () => {
      toastSuccess({ message: localization.teamCreated });
      onOpenChange(false);
    },
  });
  const updateTeam = useUpdateTeam(authClient, {
    onSuccess: () => {
      toastSuccess({ message: localization.teamUpdated });
      onOpenChange(false);
    },
  });
  const removeTeam = useRemoveTeam(authClient, {
    onSuccess: () => {
      toastSuccess({ message: localization.teamDeleted });
      onOpenChange(false);
    },
  });
          ...
          <form.AuthFormRoot className="flex flex-col gap-6">
            ...
            {team && canListMembers && (
              <div className="flex flex-col gap-4 border-t pt-5">
                <div>
                  <h3 className="font-medium text-sm">{localization.teamMembers}</h3>
                  <p className="text-muted-foreground text-sm">{localization.addTeamMember}</p>
                </div>
            ...
            {team && !canRemoveTeam && canDelete.data?.success && (
              <p className="text-muted-foreground text-sm">{teamRemovalDisabledReason}</p>
            )}
