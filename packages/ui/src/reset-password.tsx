...
import { Field, FieldDescription, FieldError, FieldGroup, FieldLabel } from "@kumix/ui/ui/field";
import {
  InputGroup,
  InputGroupAddon,
  InputGroupButton,
...
export function ResetPassword({ className }: ResetPasswordProps) {
  ...
  const { mutateAsync: resetPassword, isPending } = useResetPassword(authClient, {
    ...
    onSuccess: () => {
      toastSuccess({ message: localization.auth.passwordResetSuccess });
  ...
  useEffect(() => {
    ...
    if (!token) {
      toastError({ message: localization.auth.invalidResetPasswordToken });
      navigate({ to: signInURL });
    }
